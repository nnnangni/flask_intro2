...
def summoner():
    ...
    tier = soup.select_one("#SummonerLayoutContent > div.tabItem.Content.SummonerLayoutContent.summonerLayout-summary > div.SideContent > div.TierBox.Box > div.SummonerRatingMedium > div.TierRankInfo > div.TierRank > span").text
    f = open("output.csv", "a+", encoding="utf-8", newline="")
    csv_f = csv.writer(f)
    csv_f.writerow([username, tier, win, lose, datetime.datetime.now()])
    f.close()
